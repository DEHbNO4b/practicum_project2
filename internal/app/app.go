...
func New(log *slog.Logger, grpcPort int, dsn string, tokenTTL time.Duration) (*App, error) {

	storage, err := postgres.New(log, dsn)
	if err != nil {
		panic(err)
	}
	...
	grpcApp, err := grpcapp.New(log, authService, keeperService, grpcPort, app)
	if err != nil {
		return nil, err
	}
